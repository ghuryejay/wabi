...
This algorithm is efficiently implemented using two techniques. First one is \textit{linear scaling}, where the contribution of a node $v$ depends linearly on the distance to the sample. Nearer the node is to the sample, lesser is the contribution of that sample for the that node is counted. Linear scaling is implemented using a simple variant of aggregation scheme proposed in Brandes algorithm. Second technique is \textit{bisection scaling} where the sample only contributes to the second half of a path. This means that for a vertex $v$, the contribution of sample is considered only if it is at atleast half distance between $s$ and $t$ away from $s$ in forward search or more than half a distance away from $t$ in backward search where $s$ and $t$ are nodes in sample. 
...
There are two crucial things needed to be handled. First is computing vertex diameter. Calculating exact VD(G) will need to solve all pair shortest  path problem. The complexity of this algorithm is $O(V^{2}logV + VE)$ by Johnson's algorithm\cite{johnson} and $\Theta(V^{3})$ by Floyd-Warshall algorithm. There exits a faster linear time 2-approximation algorithm to calculate approximate vertex diameter for unweighted graph. For weighted graph, there is no relationship between VD(G) and $diam$(G) as shortest path with maximum size may not have maximum weight. For weighted graphs, VD(G) is approximated by calculating the size of largest Weekly Connected Componet(WCC). This can be done in $O(m+n)$ using Breadth First Search.  

Second is to sample shortest paths. Assume that two vertices $u$ and $v$ are connected. Shortest paths are sampled from backward using weighted random sampling. Let $S$ we the path we want to sample. Initially, $S = \{v\}$. For all nodes that are predecessors of $v$ on shortest paths from $u$ to $v$, each node will have different probability to get sampled. This probability can be calculated using method similar to aggregation method used in Brandes algorithm.  We keep on adding nodes to $S$ until we hit the source node $u$. In this way a single shortest path is sampled from all shortest paths between $u$ and $v$.
